# Julius.AI Clone — Staff-Engineer Portfolio Build

## Why this repo exists (read first)

This is a **portfolio project to earn a referral** for a role at **Julius.AI**:
...
A recruiter has said she will refer the candidate **if he builds something real with Julius's stack that she can share.** So the goal is NOT a big feature-complete product. The goal is a **small, deep, production-deployed vertical slice that reads as staff-level engineering** to a Julius engineer looking at the repo and the live site.
...
- **The gap to close:** the Julius stack is **frontend-heavy** (Next.js/React/TS/Tailwind), which is his "also-shipped," not his interview-depth. This project must show he can **ship solid React/frontend fundamentals** (not vibecoding), while his backend/systems strength turns the hard part (the Python sandbox) from a risk into the differentiator.
...
## What Julius.AI actually is (clone target)
...
**Research finding (2026-08-28):** Julius.ai is an MCP **client** — it *consumes* external MCP servers as data connectors — but it does **not** expose itself as an MCP server, and it has **no public API** ("Julius itself cannot be called via API," per their docs). So **building a Julius-as-MCP-server is a genuine gap you're filling**, in the exact direction Julius already cares about. This is the strongest single pitch in the whole project: *"an external agent can pilot the entire platform — the thing Julius doesn't ship yet."*
...
2. **A working MCP server** exposing the tool set above, driving the same core service as the UI. This is the differentiator — Julius has no such server or public API.
...
The JD explicitly wants UIs that **"don't look like AI generated."** So: restrained palette, real spacing rhythm, one good font, subtle motion, empty/loading/error states designed on purpose, charts that look publication-quality (Julius's signature). This is disproportionately visible to a reviewer in 10 seconds — invest a focused pass, don't over-build.
...
- **Never let the model fabricate results.** Any number, table, or chart must come from executed code over the real uploaded data. This is both a correctness principle and a Julius product principle — treat any model-produced fact as unverified until the sandbox produced it.
