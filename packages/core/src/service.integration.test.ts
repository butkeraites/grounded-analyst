...
test("upload: stores, profiles in the sandbox, and persists the dataset", { skip: !READY }, async () => {
  const dir = mkdtempSync(join(tmpdir(), "grounded-svc-"));
  const repos = makeRepositories(getDb(DATABASE_URL!));
  const service = createCoreService({
    repos,
    storage: new LocalStorage(dir),
    sandbox: new DockerSandbox({ datasetsMount: dir, image: "grounded-sandbox:latest" }),
    profiler: new DockerSandbox({ datasetsMount: dir, image: "grounded-sandbox:latest" }),
    llm: () => {
