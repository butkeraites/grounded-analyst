...
function dockerReady(): boolean {
  try {
    execFileSync("docker", ["image", "inspect", "grounded-sandbox:latest"], { stdio: "ignore" });
    return true;
  } catch {
    return false;
  }
...
before(() => {
  if (!READY) return;
  dir = mkdtempSync(join(tmpdir(), "grounded-sbx-"));
  writeFileSync(join(dir, "sales.csv"), CSV);
});

function sandbox() {
  return new DockerSandbox({ datasetsMount: dir, image: "grounded-sandbox:latest", defaultTimeoutMs: 20_000 });
}
...
test("a run exceeding the timeout is killed", { skip: !READY }, async () => {
  const res = await new DockerSandbox({ datasetsMount: dir, image: "grounded-sandbox:latest" }).execute({
    datasetFile: "sales.csv",
