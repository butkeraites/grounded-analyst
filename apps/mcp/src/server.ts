...
import { z } from "zod";
import { createCoreServiceFromEnv, RedisLlmBridge } from "@grounded/core";

/**
 * Grounded-clone MCP server — the capability surface Grounded itself doesn't ship.
...
const server = new McpServer({ name: "grounded-analyst", version: "0.1.0" });
...
console.error("[mcp] grounded-analyst server up (analyst + llm-worker tools)");
