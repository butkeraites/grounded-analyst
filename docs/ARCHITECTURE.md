...
flowchart TB
  ...
  subgraph core ["@grounded/core — the callable core"]
...
- **Never fabricate.** The interpretation is written *only* from executed
  ...
  are ever invented. This is both a correctness rule and a Grounded product
  principle.
...
- **Larger files & live DB connectors** — stream/chunk ingestion beyond in‑memory
  CSV; add read‑only warehouse connectors (a known Grounded entry‑tier gap).
